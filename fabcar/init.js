...
async function invoke(fabric_client, channel, request, tx_id) {
	...
	var order = fabric_client.newOrderer('grpc://47.100.192.19:7050')
	...
	*/
	...
	if (isProposalGood) {
		...
		event_hub.setPeerAddr('grpc://47.100.192.19:7053');
		...
		var _results = await Promise.all(promises);
	} else {
	...
	if (_results && _results[0] && _results[0].status === 'SUCCESS') {
		console.log('Successfully sent transaction to the orderer.');
	} else {
		console.error('Failed to order the transaction. Error code: ' + response.status);
	}

	if (_results && _results[1] && _results[1].event_status === 'VALID') {
